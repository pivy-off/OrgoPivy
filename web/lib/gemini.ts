...
export async function geminiAsk(
  ...
  const res = await fetch(apiUrl("/gemini/ask"), {
    method: "POST",
...
export async function geminiStudyGuide(topic: Topic | undefined, slug: string): Promise<{ markdown: string }> {
  incrementGlobalAiRequests();
  touchStreakActivity();
  const tp = topicPayload(topic);
  const res = await fetch(apiUrl("/gemini/study-guide"), {
    method: "POST",
...
export async function geminiAudioBrief(topic: Topic | undefined, slug: string): Promise<{ transcript: string }> {
  incrementGlobalAiRequests();
  touchStreakActivity();
  const tp = topicPayload(topic);
  const res = await fetch(apiUrl("/gemini/audio-brief"), {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      topic_slug: slug,
...
export async function geminiFreshQuestions(
  ...
  touchStreakActivity();
  const tp = topicPayload(topic);
  const res = await fetch(apiUrl("/gemini/fresh-questions"), {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      topic_slug: slug,
...
export async function geminiExplainMistake(body: {
  topicSlug: string;
  question: string;
  wrongAnswer: string;
  ...
  incrementGlobalAiRequests();
  touchStreakActivity();
  const res = await fetch(apiUrl("/gemini/explain-mistake"), {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
